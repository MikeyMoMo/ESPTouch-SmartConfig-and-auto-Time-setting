...
#include <WiFi.h>
String chSSID; //     = "MikeysWAP";        // Your router SSID.
String chPassword; // = "Noogly99";         // Your router password.
bool   CredsSet;
...
time_t TS_Epoch = 0, UTC;  // Set by Time Sync, printed by loop if not 0.
struct tm timeinfo;
...
/*******************************************************************************************/
{
  ...
  for (int i = 0; i < zoneCt; i++) {
    ...
    //    if (!strcmp(zones[i].name, loc.timezone)) {  // Try for a match in the zones table.
    if (STRCMP(zones[i].name, loc.timezone)) {
      // if (zones[i].zones == loc.timezone) {
      Serial.println("Found a match.");
      configTzTime(zones[i].zones, "pool.ntp.org");  // Use the env string from the match.
      zoneFound = true;
      break;
    }
  }
...
/*******************************************************************************************/
{
  ...
  // looper init'd to 100.  If still there, there was some error. Use SmartConfig.
